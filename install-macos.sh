...
PROVIDERS_JSON="${RELAY_AI_HOME}/providers.json"
SECRETS_JSON="${RELAY_AI_HOME}/secrets.json"
...
# 1. Записываем secrets.json для авторизации keyring
mkdir -p "$RELAY_AI_HOME"
if [ ! -f "$SECRETS_JSON" ] || ! grep -q '"provider:custom-antigravity"' "$SECRETS_JSON" 2>/dev/null; then
  cat > "$SECRETS_JSON" <<'EOP'
{
  "version": 1,
  "accounts": {
    "provider:custom-antigravity": "antigravity"
  }
}
EOP
fi

# 2. Гарантируем корректный providers.json с 19 моделями и валидными полями (addedAt)
if [ ! -f "$PROVIDERS_JSON" ] || ! grep -q '"addedAt"' "$PROVIDERS_JSON" 2>/dev/null || ! grep -q '"gemini-3.6-flash-high"' "$PROVIDERS_JSON" 2>/dev/null; then
...
# 3. Проверяем привязанные аккаунты Google
